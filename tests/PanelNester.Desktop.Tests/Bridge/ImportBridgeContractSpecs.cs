...
public sealed class ImportBridgeContractSpecs
{
    [Fact]
    public void Phase_four_import_bridge_message_names_follow_the_existing_request_response_pattern()
    {
        var responseTypes = Phase04ImportBridgeExpectations.ImportMessageTypes
        ...
        Assert.Equal(
            [
                ...
                "update-part-row-response",
                "delete-part-row-response",
                "add-part-row-response"
            ],
            responseTypes);
    }

    [Fact]
    public void Handshake_and_dispatcher_expose_phase_four_import_and_edit_operations()
    {
        var dispatcher = DesktopBridgeRegistration.CreateDefault(
            new StubFileDialogService(),
            new StubMaterialService(),
            new ProjectService(new StubMaterialService(), idGenerator: () => "project-import-contract"),
        ...
        foreach (var messageType in Phase04ImportBridgeExpectations.ImportMessageTypes)
        {
