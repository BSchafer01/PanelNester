...
public sealed class WebViewBridgeReadinessSpecs
{
    ...
    [Fact]
    public async Task Save_project_before_close_waits_for_ui_ready_and_returns_the_web_result()
    {
        var completion = new TaskCompletionSource<(bool ScriptStartedBeforeReady, DesktopCloseSaveResult SaveResult)>(
            TaskCreationOptions.RunContinuationsAsynchronously);
        var thread = new Thread(() =>
        {
            try
            {
                ...
                var bridge = new WebViewBridge(
                    ...
                    dispatcher,
                    new WebUiContentLocation("F:\\mock-ui", "Mock UI build", true),
                    Path.Combine(Path.GetTempPath(), $"PanelNester.WebViewBridgeCloseSave.{Guid.NewGuid():N}"));
                ...
                var saveTask = bridge.SaveProjectBeforeCloseAsync();
                ...
                scriptStarted.Task.Wait(TimeSpan.FromSeconds(1));
                scriptResult.TrySetResult("""{"status":"saved","message":"Saved project."}""");

                var saveResult = saveTask.WaitAsync(TimeSpan.FromSeconds(1)).GetAwaiter().GetResult();
                completion.TrySetResult((startedBeforeReady, saveResult));
            }
            catch (Exception ex)
            {
                completion.TrySetException(ex);
        ...
        var (scriptStartedBeforeReady, saveResult) = await completion.Task.WaitAsync(TimeSpan.FromSeconds(3));
        thread.Join(TimeSpan.FromSeconds(1));

        Assert.False(scriptStartedBeforeReady);
        Assert.True(saveResult.Saved);
        Assert.Equal("Saved project.", saveResult.Message);
    }
}
