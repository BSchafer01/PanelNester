...
- 2026-03-18: Brandon asked to finish the material-library location work on the repository/domain seam, not by expanding WPF-only state. `src\PanelNester.Services\Materials\JsonMaterialRepository.cs` now owns path normalization, persisted-location recovery, and default fallback while keeping bridge-facing failures explicit.
- 2026-03-18: When a repository owns a persisted custom file path, treat explicit repoint and implicit reload as different flows: `RepointAsync` may seed a missing chosen `.json`, but normal loads and `GetLocationAsync` should fall back to the default library if a stored custom file is missing/corrupt and best-effort clear the override. Key regression coverage lives in `tests\PanelNester.Services.Tests\Materials\MaterialLibraryLocationSpecs.cs` and helper `tests\PanelNester.Services.Tests\Specifications\MaterialLibraryLocationSpec.cs`.
